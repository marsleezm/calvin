...
Value StorageManager::ReadValue(const Key& key, bool new_obj) {
	if(abort_bit_ > num_aborted_){
		...
		if (node ==  configuration_->this_node_id){
			...
			if (read_set_.count(key) == false){
				Value result = actual_storage_->ReadObject(key, txn_->local_txn_id(), &abort_bit_, &local_aborted_,
	...
	if(abort_bit_ > num_aborted_){
		...
		if (node ==  configuration_->this_node_id){
			...
			if(read_set_.count(key)){
				...
				Value result = actual_storage_->ReadLock(key, txn_->local_txn_id(), &abort_bit_, &local_aborted_,
