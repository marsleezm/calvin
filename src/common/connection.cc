...
ConnectionMultiplexer::~ConnectionMultiplexer() {
  ...
  // Close tcp sockets.
  delete remote_in_;
  for (std::tr1::unordered_map<int, zmq::socket_t*>::iterator it = remote_out_.begin();
  ...
  delete inproc_in_;
  for (std::tr1::unordered_map<string, zmq::socket_t*>::iterator it = inproc_out_.begin();
       it != inproc_out_.end(); ++it) {
    delete it->second;
  }
  
  string prefix = "scheduler";
  for (std::tr1::unordered_map<string, AtomicQueue<MessageProto>*>::iterator it = remote_result_.begin();
