...
class TxnScheduler{
	public:
		TxnScheduler(int num_all_nodes, int this_node_id, Connection* conn, AtomicQueue<TxnProto*> txn_queue) 
			...
			if (need_spt == false and (batch_msgs[batch_id%num_nodes] or getBatch(batch_id))){
				...
				while(true){
                    if(txn_idx < batch_msgs[batch_id%num_nodes].data_size()){
                        ...
                        if(idx != this_node or (prev_inv_node == -1 or prev_inv_node == txn->involved_nodes())){
	...
	private:
        ...
        int fetched_num = 0;
        AtomicQueue<TxnProto*> txn_queue;
}
