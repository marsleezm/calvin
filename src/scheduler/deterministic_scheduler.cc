...
void* DeterministicScheduler::RunWorkerThread(void* arg) {
    ...
    std::tr1::unordered_map<int64_t, StorageManager*> active_l_tids;

    queue<MyTuple<int64, int, StorageManager*>> retry_txns;

    int max_sc = scheduler->max_sc; 
    int64 this_node = scheduler->configuration_->this_node_id; //prevtx=0; 
    ...
    while (!terminated_) {
        if (!my_to_sc_txns->empty()){
            ...
            if (to_sc_txn.second == num_lc_txns_){
                ...
                if (mgr->CanSCToCommit() == SUCCESS){
                    LOG(to_sc_txn.first,  " committed! Max commit ts is "<<num_lc_txns_);
                    ++num_lc_txns_;
                    ...
                    LOCKLOG(to_sc_txn.first, " tid is "<<to_sc_txn.second<<", "<<reinterpret_cast<int64>(mgr)<<" is popped out of sc, values are "<<mgr->spec_committed_<<", "<<mgr->abort_bit_);
                    my_to_sc_txns->pop();
                }
            }
            else if (to_sc_txn.second < num_lc_txns_)
                my_to_sc_txns->pop();
        }
      ...
      if(!waiting_queue.Empty()){
          ...
          if(to_wait_txn.first >= num_lc_txns_){
              ...
              if (manager && manager->TryToResume(to_wait_txn.second, to_wait_txn.third)){
                  if(scheduler->ExecuteTxn(manager, thread, active_l_tids) == false){
      ...
      if(retry_txns.size()){
          ...
          if(retry_txns.front().first < num_lc_txns_ || retry_txns.front().second <= retry_txns.front().third->num_executed_){
              ...
              if(scheduler->ExecuteTxn(retry_txns.front().third, thread, active_l_tids) == true)
                  retry_txns.pop();
              else
...
bool DeterministicScheduler::ExecuteCommitTxn(StorageManager* manager, int thread){
    ...
    if(manager->ReadOnly()){
        LOG(txn->txn_id(), " read-only, num lc "<<num_lc_txns_);
        ++num_lc_txns_;
...
bool DeterministicScheduler::ExecuteTxn(StorageManager* manager, int thread, std::tr1::unordered_map<int64_t, StorageManager*>& active_l_tids){
    TxnProto* txn = manager->get_txn();
    if(manager->ReadOnly()){
        ...
        if (num_lc_txns_ == txn->local_txn_id()){
            ...
            AGGRLOG(txn->txn_id(), " sc"<< txn->local_txn_id()<<", nlc:"<<num_lc_txns_);
