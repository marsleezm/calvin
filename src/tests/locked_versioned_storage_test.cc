...
TEST(ReadTest, TwoReadCommit) {
	...
	v2 = storage->ReadObject("2", 0, NULL, 0, NULL, NULL, NULL);

	ASSERT_EQ(NULL, v1);
	ASSERT_EQ(NULL, v2);

	v1 = storage->ReadObject("1", 2, NULL, 0, NULL, NULL, NULL);
	v2 = storage->ReadObject("2", 2, NULL, 0, NULL, NULL, NULL);
...
TEST(ReadTest, ReadBlockCommit) {
	...
	storage->PutObject("1", "10", 1, false);
	ASSERT_TRUE("10" == v4);
